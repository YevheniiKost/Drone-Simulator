...
using PIDController = DroneSimulator.PIDController;

namespace DroneSimulator.Flight
{
    // Pure flight logic. Has no Unity MonoBehaviour dependency and no input or
    // Rigidbody access — it only transforms sensor readings + player intent
    // into per-motor force values that the view layer can then apply.
    public class QuadcopterFlightController
    {
        ...
        {
            UpdateThrottle(inputState.ThrottleDelta);
            ...
            // Yaw: steer by adding torque in the requested direction, then damp
            // any residual angular velocity that the PID picks up as an error.
        ...
        // When the drone is above half throttle it is more responsive, so we
        // double the pitch gains to keep the stabilisation effective.
        ...
        // Each propeller starts at the base throttle level.
        // The PID corrections tilt the thrust plane to level the drone.
        // The steering offsets tilt it intentionally to move in the desired direction.
